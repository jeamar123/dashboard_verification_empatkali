<script>
	...
	var Pending = {
		...
		methods: {
			checkUserRequestTransactions(){
				let vm = this
				let userReq = JSON.parse(localStorage.getItem('emitUserRequestStatus'));
				console.log(userReq);
				vm.userReqAlert = userReq ? { isShow: true, ...userReq } : { isShow: false };
			...
			closeAlert(){
				let vm = this
				vm.userReqAlert.isShow = false;
				localStorage.setItem('emitUserRequestStatus', null);
			},
      decodeJwt(paramToken) {
			...
			goToUserDetails(data)	{
				let vm = this
				vm.$router.push({ name: 'User Details', params: { status: 'pending', id: data._id } });
			},
			async getAllUsers() {
				let vm = this
			...
			async totalUsers(queryStringObj) {
				let vm = this
				try {
					let url = `/api/users?status=1&skip=0&limit=10000${ (queryStringObj!==undefined)?`&${Object.keys(queryStringObj)}=${Object.values(queryStringObj)}`:'' }`;
			...
			async getUserList(page, queryStringObj) {
				...
				let url = `/api/users?status=1&skip=${skip}&limit=${vm.paginationData.perPage}${ (queryStringObj!==undefined)?`&${Object.keys(queryStringObj)}=${Object.values(queryStringObj)}`:'' }`
				vm.toggleLoader(true, 'Loading data');
