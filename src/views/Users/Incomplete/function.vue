	...
	var Incomplete = {
      ...
      decodeJwt(paramToken) {
			...
			goToUserDetails(data)	{
				let vm = this
				vm.$router.push({ name: 'User Details', params: { status: 'incomplete', id: data._id } });
			},
			async getAllUsers() {
				let vm = this
			...
			async totalUsers(queryStringObj) {
				let vm = this
				try {
					let url = `/api/users?status=0&skip=0&limit=10000${ (queryStringObj!==undefined)?`&${Object.keys(queryStringObj)}=${Object.values(queryStringObj)}`:'' }`;
			...
			async getUserList(page, queryStringObj) {
				...
				let url = `/api/users?status=0&skip=${skip}&limit=${vm.paginationData.perPage}${ (queryStringObj!==undefined)?`&${Object.keys(queryStringObj)}=${Object.values(queryStringObj)}`:'' }`
				vm.toggleLoader(true, 'Loading data');
