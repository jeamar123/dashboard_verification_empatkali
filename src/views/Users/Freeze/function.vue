	...
	var Freeze = {
      ...
      decodeJwt(paramToken) {
			...
			async totalUsers(queryStringObj) {
				let vm = this
				try {
					let url = `/api/users?status=5&skip=0&limit=10000${ (queryStringObj!==undefined)?`&${Object.keys(queryStringObj)}=${Object.values(queryStringObj)}`:'' }`;
			...
			async getUserList(page, queryStringObj) {
				...
				let url = `/api/users?status=5&skip=${skip}&limit=${vm.paginationData.perPage}${ (queryStringObj && queryStringObj.searchFilterObj !==undefined)?`&${Object.keys(queryStringObj.searchFilterObj)}=${Object.values(queryStringObj.searchFilterObj)}`:'' }`
			...
			async refreshData(opt)	{
				...
				if(opt && opt == 'freeze'){
					vm.isFreezeStatusAlertShow = true;
					setTimeout(()	=>	{
						vm.closeAlert();
					},10000);
				}
			...
			closeAlert(){
				let vm = this
				vm.isFreezeStatusAlertShow = false;
			},
    }
	}
