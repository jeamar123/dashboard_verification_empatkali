<template>
  <div>
    ...
    <div class="flex mb-3">
      ...
      <p class="flex-1 text-sm">{{ user.detail ? user.detail.email : '---' }}</p>
    </div>

    <div class="text-right mt-8">
      <button @click.prevent="closeModal(false, 'freeze')" class="btn px-4 py-2 mx-1 text-violet font-bold rounded-md text-sm">Batal</button>
      <button @click.prevent="unFreezeUser" class="btn px-6 py-3 mx-1 bg-violet font-bold text-white rounded-md text-sm">UnFreeze</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    unFreezeUser(){
      ...
      axios.post(`/api/users/unfreezeuser`, data, vm.requestedHeaders)
        .then((res)	=>	{
          console.log(res);
          vm.requestSuccess('freeze');
        })
        .catch((err)	=>	{
          console.log(err);
