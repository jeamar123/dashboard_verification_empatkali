	...
	var Rejected = {
      ...
      decodeJwt(paramToken) {
			...
			async getUserList(page, queryStringObj) {
				...
				let url = `/api/users?status=4&skip=${skip}&limit=${vm.paginationData.perPage}${ (queryStringObj && queryStringObj.searchFilterObj !==undefined)?`&${Object.keys(queryStringObj.searchFilterObj)}=${Object.values(queryStringObj.searchFilterObj)}`:'' }`
